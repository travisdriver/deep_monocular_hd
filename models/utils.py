...
class conv2DGroupNorm(nn.Module):
    def __init__(self, in_channels, n_filters, k_size, stride, padding, bias=False, dilation=1, n_groups=16):
...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm):
        ...
        if is_batchnorm:
            ...
            self.conv1 = conv2DBatchNormRelu(in_size, out_size, 3, 1, 1, is_batchnorm=False)
            self.conv2 = conv2DBatchNormRelu(out_size, out_size, 3, 1, 1, is_batchnorm=False)
    ...
    def __init__(
        ...
        if include_range in ["all", "identity"]:
            for i in range(n_blocks - 1):
                layers.append(
                    bottleNeckIdentifyPSP(out_channels, mid_channels, stride, dilation, is_batchnorm=is_batchnorm)
                )
...
class cascadeFeatureFusion(nn.Module):
    ...
    def forward(self, x_low, x_high):
        ...
        """
        _, _, h, w = x_high.size()
        x_low_upsampled = F.interpolate(x_low, size=(h, w), mode="bilinear", align_corners=True)
